...
void NPCAIMgr::Process() {
	...
	case Chasing: {
		...
		if(m_entityAttackRange2 > m_npc->DistanceTo2(target)) {
	...
	case Following: {
		...
		if(m_entityChaseMaxDistance2 < m_npc->DistanceTo2(target)) {
...
void NPCAIMgr::Targeted(SystemEntity *by_who) {
	...
	case Idle: {
		...
		if(m_entityAttackRange2 > dist) {
			_EnterEngaged(by_who);
		} else if(m_entityChaseMaxDistance2 > dist) {
