...
public:
	...
	/** Sets write position
	 */
	ASCENT_INLINE void resize(size_t size)
	{
		m_writePos = size;
	}
	 ...
	 */
	ASCENT_INLINE const uint8 * contents()
	{
		return m_buffer;
	}
...
#define DEFINE_FAST_WRITE_OPERATOR(type, size) ByteBuffer& operator << (const type src) { if(m_writePos + size > m_buffersize) { reserve(m_buffersize + DEFAULT_INCREASE_SIZE); } *(type*)&m_buffer[m_writePos] = src; m_writePos += size; return *this; }
	...
	*/
	 ...
	 * @return buffer size
	 */
	ASCENT_INLINE size_t size() { return m_writePos; }

	/** read/write position setting/getting
	 */
	ASCENT_INLINE size_t rpos() { return m_readPos; }
	ASCENT_INLINE size_t wpos() { return m_writePos; }
	ASCENT_INLINE void rpos(size_t p) { ASSERT(p <= m_writePos); m_readPos = p; }
	ASCENT_INLINE void wpos(size_t p) { ASSERT(p <= m_buffersize); m_writePos = p; }
};
