...
void ConfigMgr::loadFromBulk()
{
    ...
    while ((file_path = bulkdataDir.NextFile()) != NULL)
    {
        ...
        */
        ...
        PyTuple * cached_item = (PyTuple *)bulk_obj;

        std::string objectID;
        if (!cached_item->GetString(0, objectID))
        {
            sLog.Error("ConfigMgr", "Unable to process cached object");
            return;
        }

        CachedObject *cachedObject = (CachedObject*)cached_item->GetItem(1);
        if(!cachedObject)
        {
            sLog.Error("ConfigMgr", "Unable to process cached object");
            return;
        ...
        // in order to destroy the rest of the object we need to increase this.
        cachedObject->IncRef();

        // destroy the tuple wrapper around our cached object
        cached_item->DecRef();
